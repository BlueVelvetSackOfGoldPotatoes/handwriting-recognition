...
def train(opt):
    ...
    # setup optimizer
    optimizer = optim.Adadelta(filtered_parameters, lr=opt.lr, rho=opt.rho, eps=opt.eps, weight_decay=1e-3)
    print("Optimizer:")
    print(optimizer)
    ...
    while(True):
        ...
        if i % opt.valInterval == 0:
            ...
            with open(f'./saved_models/{opt.experiment_name}/log_train.txt', 'a') as log:
                ...
                print('-' * 155)
                print(f'{"Ground Truth":75s} | {"Prediction":75s} | Confidence Score & T/F')
                log.write(f'{"Ground Truth":75s} | {"Prediction":75s} | {"Confidence Score"}\n')
                print('-' * 155)
                for gt, pred, confidence in zip(labels[:5], preds[:5], confidence_score[:5]):
                    print(f'{gt:75s} | {pred:75s} | {confidence:0.4f}\t{str(pred == gt)}')
                    log.write(f'{gt:75s} | {pred:75s} | {confidence:0.4f}\t{str(pred == gt)}\n')
                print('-' * 155)

        # save model per 5e+4 iter.
        if (i + 1) % 5e+4 == 0:
            torch.save(
...
if __name__ == '__main__':
    ...
    if not opt.experiment_name:
        opt.experiment_name = 'VGG-BiLSTM-CTC'
